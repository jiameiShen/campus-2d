        ...
        245: (e, t, a) => {
          ...
          const r = i.graphic.extendShape({
            ...
            buildPath: function (e, t) {
              ...
              for (
                var a = Math.max(2 * Math.ceil(((2 * t.radius) / t.waveLength) * 4), 8);
                t.phase < 2 * -Math.PI;
          ...
          i.extendChartView({
            ...
            render: function (e, t, a) {
              ...
              var o = e.getData(),
                ...
                g = 0,
                v = 0,
                f = e.get('outline.show')
              f &&
                ((g = e.get('outline.borderDistance')),
                (v = l(e.get('outline.itemStyle.borderWidth'), c)))
              var m,
                y,
                w,
                M = l(h[0], p),
              ...
              'container' === x
                ? ((P = !0),
                  (y = [(m = [p / 2, u / 2])[0] - v / 2, m[1] - v / 2]),
                  (w = [l(g, p), l(g, u)]),
                  (d = [Math.max(y[0] - w[0], 0), Math.max(y[1] - w[1], 0)]))
                : ((y = (m = l(d, c) / 2) - v / 2), (w = l(g, c)), (d = Math.max(y - w, 0))),
                f && ((F().style.lineWidth = v), n.add(F()))
              var I = P ? 0 : M - d,
                S = P ? 0 : b - d,
                L = null
              n.add(
              ...
              function D(e, t) {
                if (x) {
                  if (0 === x.indexOf('path://')) {
                    var a = i.graphic.makePath(x.slice(7), {}),
                      r = a.getBoundingRect(),
                      n = r.width,
                      l = r.height
                    ...
                    return (
                      (a = i.graphic.makePath(
                        x.slice(7),
                        {},
                        new i.graphic.BoundingRect(o, s, n, l)
                      )),
                      t && (a.position = [-n / 2, -l / 2]),
                      a
                    )
                  }
                  if (P) {
                    var h = t ? -e[0] : M - e[0],
                      d = t ? -e[1] : b - e[1]
                    return i.helper.createSymbol('rect', h, d, 2 * e[0], 2 * e[1])
                  }
                  return (
                    (h = t ? -e : M - e),
                    (d = t ? -e : b - e),
                    'pin' === x ? (d += e) : 'arrow' === x && (d -= e),
                    i.helper.createSymbol(x, h, d, 2 * e, 2 * e)
                  )
                }
                return new i.graphic.Circle({ shape: { cx: t ? 0 : M, cy: t ? 0 : b, r: e } })
              }
              function F() {
                var t = D(m)
                return (
              ...
              function Y(t, a, n) {
                var s = P ? d[0] : d,
                  h = P ? u / 2 : d,
                  p = o.getItemModel(t),
                  c = p.getModel('itemStyle'),
                  g = p.get('phase'),
                  v = l(p.get('amplitude'), 2 * h),
                  f = l(p.get('waveLength'), 2 * s),
                  m = h - o.get('value', t) * h * 2
                g = n ? n.shape.phase : 'auto' === g ? (t * Math.PI) / 4 : g
                var y = c.getItemStyle()
                if (!y.fill) {
                  var w = e.get('color'),
                    x = t % w.length
                  y.fill = w[x]
                ...
                I.shape._waterLevel = m
                var S = p.getModel('emphasis.itemStyle').getItemStyle()
                ;(S.lineWidth = 0),
                  (I.ensureState('emphasis').style = S),
                  i.helper.enableHoverEmphasis(I)
                var L = D(d, !0)
                return L.setStyle({ fill: 'white' }), I.setClipPath(L), I
              }
              function E(e, t, a) {
                var i = o.getItemModel(e),
                  r = i.get('period'),
                  n = i.get('direction'),
                  l = o.get('value', e),
                  s = i.get('phase')
                s = a ? a.shape.phase : 'auto' === s ? (e * Math.PI) / 4 : s
                var h, d
                h =
                  'auto' === r
                    ? 0 === (d = o.count())
              ...
              o
                ...
                .update(function (t, a) {
                  for (
                    ...
                    var p = h[d]
                    l.shape.hasOwnProperty(p) && (s[p] = l.shape[p])
                  }
                  var c = {},
                    g = ['fill', 'opacity', 'shadowBlur', 'shadowColor']
                  for (d = 0; d < g.length; ++d)
                    (p = g[d]), l.style.hasOwnProperty(p) && (c[p] = l.style[p])
                  P && (s.radiusY = u / 2),
                    i.graphic.updateProps(r, { shape: s }, e),
                    r.useStyle(c),
                ...
                s.get('label.show') &&
                  n.add(
                    (function (t) {
                      ...
                      Object.assign(p.style, i.helper.createTextStyle(a))
                      var u = new i.graphic.Rect(h),
                        c = new i.graphic.Rect(h)
                      ;(c.disableLabelAnimation = !0), (u.disableLabelAnimation = !0)
                      var g = new i.graphic.Text(p),
                        v = new i.graphic.Text(p)
                      u.setTextContent(g), c.setTextContent(v)
                      var f = a.get('insideColor')
                      v.style.fill = f
                      var m = new i.graphic.Group()
                      m.add(u), m.add(c)
                      var y = D(d, !0)
                      return (
                        (L = new i.graphic.CompoundPath({
                          shape: { paths: t },
                          position: [M, b],
                        })).setClipPath(y),
                        c.setClipPath(L),
